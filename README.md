...
- 两相线性ROM与Turner体积模量加权模型的零热膨胀配比计算；
...
- 基于真实PTE/NTE `alpha(T)` 曲线的温区ZTE设计：比较线性ROM与Turner模型，自动读取体积模量和结构密度，输出体积/质量分数、误差、ZTE覆盖率、连续满足温区及完整混合曲线；
...
- ZTE页面从数据库内结构自动计算两相密度，用于体积分数到质量分数换算；Turner模型使用数据库体积模量进行加权。体积模量视为常数且配比无额外约束时，两种模型可能以不同体积分数获得相同的最优热权重，因此应重点比较所需配比和物理假设，而不能仅凭拟合误差判断模型优劣。两种模型都是理想两相估算，不包含孔隙率、界面反应、织构和微裂纹效应，实验配方仍需进一步验证。
