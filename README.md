...
当前版本：`0.10.0`（6701条活跃NTE数据库、185条PTE参考数据、三维结构、真实QHA曲线、ZTE设计与受控Agent）
...
- ZTE全库筛选在统一温度网格上严格评估185×6701个潜在组合，支持温区、目标容差、模型、基体、材料限定词、NTE体积分数及工程可实现性约束，并按覆盖率、最长连续温区和误差返回完整排名；工程约束可要求组合包含/排除指定元素、两相密度与K/G数据完整，并限制密度、体积模量和剪切模量失配比；候选可一键回填到手动设计，也可勾选最多6组进行真实混合曲线叠加、Pareto多目标比较及CSV、JSON、PDF导出；Pareto点与排名表支持悬停、单击选择和定位联动；
