...
- ZTE全库筛选在统一温度网格上严格评估185×6701个潜在组合，支持温区、目标容差、模型、基体、材料限定词及NTE体积分数约束，并按覆盖率、最长连续温区和误差返回完整排名；候选可一键回填到手动设计，也可勾选最多6组进行真实混合曲线叠加、Pareto多目标比较及CSV、JSON、PDF导出；Pareto点与排名表支持悬停、单击选择和定位联动；
