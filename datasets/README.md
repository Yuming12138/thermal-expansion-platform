...
`nte-candidates-6702-v1`包含6702条唯一材料记录，每条记录均包含POSCAR结构和25项派生属性。
...
原始目录约5.7 GB，并包含大量QHA图、PDF及中间数据。为保持仓库清洁，V1只纳入完整的结构与属性JSON压缩快照；原始QHA结果目录不复制进Git，通过清单中的路径和校验和追溯。
