...
平台已经采用双数据库运行模式：`catalog-v1.sqlite` 使用 SQLite `mode=ro&immutable=1` 只读连接，材料查询、结构、历史曲线和 ZTE 配对只访问该目录库；用户上传、计算任务与结果写入独立的 `workspace.sqlite`。运行时不复制或修改目录库，也不再回退到原始科研数据目录。
